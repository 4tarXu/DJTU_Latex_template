...
%==========================页面边距设置=============================
\usepackage[a4paper,left=20mm,right=20mm,top=25mm,bottom=20mm,headheight=16mm,headsep=4mm ,footskip=15mm]{geometry}	
...
\renewcommand{\headrule}{%
  \hrule width\headwidth height 1.5pt \vskip 0.5pt% 粗线（1.5磅）
  \hrule width\headwidth height 0.4pt% 细线（0.4磅）
 \vskip 0.5pt% 间距
}
\makeatother
...
\fancypagestyle{djtu}{
	...
	\fancyhead[CE]{\zihao{-5} \songti 大连交通大学专业学位硕士学位论文}  % 偶数页眉，居中，小五，黑体 
	\fancyhead[CO]{\zihao{-5} \songti \leftmark}  % 奇数页眉，居中，小五，黑体
	}
 \fancypagestyle{plain}{
 	\fancyhf{}		% 清空所有定义
 	\fancyfoot[CE,CO]{\zihao{-5} \songti \thepage}      % 设置页脚为当前页码，居中,小五，数字1，2，3等
 	\fancyhead[CE]{\zihao{-5} \songti 大连交通大学专业学位硕士学位论文}  % 偶数页眉，居中，小五，宋体
 	\fancyhead[CO]{\zihao{-5} \songti \leftmark}  % 奇数页眉，居中，小五，宋体
     }
% 定义中文摘要部分的页面样式：
\fancypagestyle{cnabstract}{
	\fancyhf{}		% 清空所有定义
	\fancyfoot[CE,CO]{\zihao{-5} \thepage}      % 设置页脚为当前页码，居中,小五
	\fancyhead[CE,CO]{\zihao{-5} \songti 摘要}  % 偶数页眉，居中，小五，黑体 
...
\fancypagestyle{enabstract}{
	\fancyhf{}		% 清空所有定义
	\fancyfoot[CE,CO]{\zihao{-5} \thepage}      % 设置页脚为当前页码，居中,小五
	\fancyhead[CE,CO]{\zihao{-5} \songti ABSTRACT}  % 偶数页眉，居中，小五，黑体 
...
\ctexset{ % 用来定制正文里的标题格式
	chapter = {%
		name = {},
		number = \normalfont\arabic{chapter},              % 用中文数字显示章节号
	...
	section = {%
        ...
        beforeskip = 10pt,	% 段前行距0行
        afterskip = 5pt,	% 段后行距0行
	},
	subsection = {%
...
\renewcommand{\contentsname}{\zihao{-3}目\quad 录}		% “目录”两字之间空X格

\titlecontents{chapter}
[0em] % 缩进量
{\songti\zihao{-4}} % 控制字体字号
{\thecontentslabel\hspace{0.25em}} % 控制章节编号和名称的距离 
{} 
...
[2em] % 缩进量
{\songti\zihao{-4}} % 控制字体字号
{\thecontentslabel\hspace{1.25em}} % 控制小节编号和名称的距离 
{} 
{\titlerule*[3pt]{.}\hspace{-1em}\contentspage} % 控制引导点密度,这里引入\hspace{-1em}的功能是防止页码和引导点之间有空格
...
[3.75em] % 缩进量
{\songti\zihao{-4}} % 控制字体字号
{\thecontentslabel\hspace{1.5em}} % 控制子节编号和名称的距离
{} 
{\titlerule*[3pt]{.}\hspace{-1em}\contentspage}% 控制引导点密度,这里引入\hspace{-1em}的功能是防止页码和引导点之间有空格
...
\captionsetup[figure]{skip=0pt} % 设置图片标题与内容之间无间距
\renewcommand{\arraystretch}{1.5} % 扩大表格每行文字距离
\DeclareCaptionLabelSeparator{quadspace}{\quad}  % 定义分隔符为\quad

\DeclareCaptionFont{myfont}{\fontsize{10.5pt}{12.6pt}\selectfont} % 自定义一个10.5pt的字体大小
\captionsetup[table]{
...
\fancypagestyle{plain}{
	\fancyhf{}		% 清空所有定义
	\fancyfoot[CE,CO]{\zihao{-5} \songti \thepage}      % 设置页脚为当前页码，居中,小五
	\fancyhead[CE,CO]{\zihao{-5} \songti ABSTRACT}  % 偶数页眉，居中，小五，宋体
	\renewcommand{\headrulewidth}{0pt}  % 去除页眉横线
	\renewcommand{\footrulewidth}{0pt}  % 去除页脚横线
	}
...
\fancypagestyle{plain}{%
    \fancyhf{}  % 清空所有页眉页脚
    \fancyfoot[CE,CO]{\zihao{-5} \songti \thepage}      % 设置页脚为当前页码，居中,小五
    \fancyhead[CE,CO]{\zihao{-5} \songti 目录}  % 偶数页眉，居中，小五，宋体
    \renewcommand{\headrulewidth}{0pt}  % 去除页眉横线
    \renewcommand{\footrulewidth}{0pt}  % 去除页脚横线
} 


\pagestyle{plain}   % 目录增加 
\tableofcontents   						% 载入目录
...
\fancypagestyle{plain}{
	\fancyhf{}		% 清空所有定义
	\fancyfoot[CE,CO]{\zihao{-5} \songti \thepage}      % 设置页脚为当前页码，居中,小五
	\fancyhead[CE,CO]{\zihao{-5} \songti 参考文献}  % 偶数页眉，居中，小五，宋体
	\renewcommand{\headrulewidth}{0pt}  % 去除页眉横线
	\renewcommand{\footrulewidth}{0pt}  % 去除页脚横线
	}
...
\renewcommand*{\bibfont}{\zihao{5}\songti}
 \printbibliography[title={\zihao{-3}参考文献},heading=bibintoc] % 导入参考文献

%% -------------------- 致谢 -------------------- 	
\fancypagestyle{plain}{
	\fancyhf{}		% 清空所有定义
	\fancyfoot[CE,CO]{\zihao{-5} \songti \thepage}      % 设置页脚为当前页码，居中,小五
	\fancyhead[CE,CO]{\zihao{-5} \songti 致谢}  % 偶数页眉，居中，小五，宋体
	\renewcommand{\headrulewidth}{0pt}  % 去除页眉横线
	\renewcommand{\footrulewidth}{0pt}  % 去除页脚横线
	}
