...
\fancypagestyle{djtu}{
	...
	\fancyhead[CE]{\zihao{-5} \heiti 大连交通大学2022级信息管理与信息系统专业学年论文}  % 偶数页眉，居中，小五，黑体 
	\fancyhead[CO]{\zihao{-5} \heiti 大连交通大学2022级信息管理与信息系统专业学年论文}  % 奇数页眉，居中，小五，黑体
	}
...
%% -------------------- 正文 -------------------- 				
% 下设三个模块，分别对应三个tex文件
%	- 第一个：信息检索与检索工具模块 ———— 对应调研报告
%		- 模版中教学内容：列表环境
%	- 第二个：文献阅读与管理工具模块 ———— 对应文献综述、外文翻译?文献可视化
%		- 模版中教学内容：图、表的插入(表格的重点在于样式设计，学会使用边框、合并单元格、三线表)
%		- 思维：通过图，找到可行的研究(探索)方向
%	- 第三个：文档编辑与规范化 ———— 对应排版
%		- 模版中使用教师发送的指定PDF进行排版
%		- 熟练使用各种环境：公式(\equation)、图片(\figure)、参考文献(cite{})

\include{latex/Information_Management.tex}    % 载入信息管理(信息检索与检索工具)模块

\include{latex/Literature_Management.tex}	  % 载入文献管理与可视化模块

\include{latex/Standardization_Documents.tex}    % 载入标准化文档模块
